...
def upgrade() -> None:
    ...
    op.create_unique_constraint("uq_parent_items_sku", "parent_items", ["sku"])

    # Add unique constraint to child_items.sku
    op.create_unique_constraint("uq_child_items_sku", "child_items", ["sku"])


def downgrade() -> None:
    """Remove unique constraints from SKU fields."""
    # Remove unique constraint from child_items.sku
    op.drop_constraint("uq_child_items_sku", "child_items", type_="unique")

    # Remove unique constraint from parent_items.sku
    op.drop_constraint("uq_parent_items_sku", "parent_items", type_="unique")
