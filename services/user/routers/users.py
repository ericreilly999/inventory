...
from ..schemas import (
    UserCreate, UserUpdate, UserResponse, MessageResponse, 
    PasswordChange
)
from ..dependencies import (
    get_current_user, require_user_read, require_user_write, 
    require_user_admin
)
...
@router.post("/", response_model=UserResponse, dependencies=[Depends(require_user_admin)])
...
@router.get("/{user_id}", response_model=UserResponse, dependencies=[Depends(require_user_read)])
async def get_user(
    user_id: UUID,
    db: Session = Depends(get_db)
):
...
@router.put("/{user_id}", response_model=UserResponse, dependencies=[Depends(require_user_write)])
...
@router.delete("/{user_id}", response_model=MessageResponse, dependencies=[Depends(require_user_admin)])
