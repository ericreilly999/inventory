...
async def cleanup_admin(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error cleaning up admin: {str(e)}")
...
async def seed_database(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        for name, description in location_types_data:
            existing_type = db.query(LocationType).filter(LocationType.name == name).first()
        ...
        for name, description, category in item_types_data:
            existing_item_type = db.query(ItemType).filter(ItemType.name == name).first()
...
async def get_seed_status(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error checking seed status: {str(e)}")
