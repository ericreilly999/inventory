...
from ..schemas import MessageResponse, RoleCreate, RoleResponse, RoleUpdate
...
async def update_role(
    ...
    if role_data.name and role_data.name != role.name:
        existing = (
            db.query(Role)
            .filter(and_(Role.name == role_data.name, Role.id != role_id))
            .first()
        )
        if existing:
            raise HTTPException(
...
async def get_role_users(role_id: UUID, db: Session = Depends(get_db)):
