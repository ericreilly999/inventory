...
from ..dependencies import require_reports_read, get_current_user
from ..schemas import (
    InventoryStatusReport, InventoryStatusByLocation, InventoryStatusRequest,
    MovementHistoryReport, MovementRecord, MovementHistoryRequest,
    InventoryCountReport, InventoryCountByType, InventoryCountByLocationAndType,
    InventoryCountRequest, LocationSummary, ItemTypeSummary, UserSummary,
    ReportError
)
...
async def get_inventory_status_report(
    ...
    try:
        ...
        for location in locations:
            ...
            parent_items_count = db.query(ParentItem).filter(
                ParentItem.current_location_id == location.id
            ).count()
            
            # Count child items (through their parent items)
            child_items_count = db.query(ChildItem).join(ParentItem).filter(
                ParentItem.current_location_id == location.id
            ).count()
            ...
            location_reports.append(InventoryStatusByLocation(
...
async def get_movement_history_report(
    ...
    try:
        ...
        for movement in movements:
            ...
            movement_records.append(MovementRecord(
                id=movement.id,
...
async def get_inventory_count_report(
    ...
    try:
        ...
        for item_type, parent_count, child_count in item_type_counts:
            by_item_type.append(InventoryCountByType(
        ...
        for location, item_type, parent_count, child_count in location_type_counts:
            if item_type:  # Only include if there's an item type
                by_location_and_type.append(InventoryCountByLocationAndType(
                    location=LocationSummary(
                        id=location.id,
                        name=location.name,
                        location_type=location.location_type.name
