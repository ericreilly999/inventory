...
from shared.models.item import ChildItem, ItemType, ParentItem, ItemCategory
from shared.models.assignment_history import AssignmentHistory
from shared.models.user import User
from shared.logging.config import get_logger
from ..schemas import (
    ChildItemCreate, ChildItemUpdate, ChildItemResponse, MessageResponse,
    AssignmentHistoryResponse
)
from ..dependencies import (
    get_current_user, require_inventory_read, require_inventory_write,
    require_inventory_admin, get_child_item_or_404, get_item_type_or_404,
    get_parent_item_or_404, validate_item_type_category
)
...
@router.post("/", response_model=ChildItemResponse, dependencies=[Depends(require_inventory_write)])
...
@router.get("/", response_model=List[ChildItemResponse], dependencies=[Depends(require_inventory_read)])
...
@router.get("/{item_id}", response_model=ChildItemResponse, dependencies=[Depends(require_inventory_read)])
...
@router.put("/{item_id}", response_model=ChildItemResponse, dependencies=[Depends(require_inventory_write)])
...
@router.delete("/{item_id}", response_model=MessageResponse, dependencies=[Depends(require_inventory_admin)])
...
@router.get("/{item_id}/assignment-history", response_model=List[AssignmentHistoryResponse], dependencies=[Depends(require_inventory_read)])
async def get_child_item_assignment_history(
    ...
    assignment_history = db.query(AssignmentHistory).filter(
        AssignmentHistory.child_item_id == item_id
    ).order_by(AssignmentHistory.assigned_at.desc()).offset(skip).limit(limit).all()
    
    return [AssignmentHistoryResponse.from_orm(assignment) for assignment in assignment_history]


@router.post("/{item_id}/reassign/{new_parent_id}", response_model=ChildItemResponse, dependencies=[Depends(require_inventory_write)])
