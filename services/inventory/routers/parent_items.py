...
from ..schemas import (
    ParentItemCreate, ParentItemUpdate, ParentItemResponse, MessageResponse,
    ItemLocationQuery
)
from ..dependencies import (
    get_current_user, require_inventory_read, require_inventory_write,
    require_inventory_admin, get_parent_item_or_404, get_item_type_or_404,
    get_location_or_404, validate_item_type_category
)
...
@router.post("/", response_model=ParentItemResponse, dependencies=[Depends(require_inventory_write)])
...
@router.get("/", response_model=List[ParentItemResponse], dependencies=[Depends(require_inventory_read)])
...
@router.get("/{item_id}", response_model=ParentItemResponse, dependencies=[Depends(require_inventory_read)])
...
@router.get("/{item_id}/location", response_model=ItemLocationQuery, dependencies=[Depends(require_inventory_read)])
...
@router.put("/{item_id}", response_model=ParentItemResponse, dependencies=[Depends(require_inventory_write)])
...
async def delete_parent_item(
    ...
    return MessageResponse(message=f"Parent item deleted successfully (including {child_count} child items)")