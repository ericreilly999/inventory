...
async def list_parent_items(
    ...
    logger.info(
        ...
        sample_item=(
            {
                "id": str(parent_items[0].id),
                "sku": parent_items[0].sku,
                "description": parent_items[0].description,
            }
            if parent_items
            else None
        ),
...
async def delete_parent_item(
    ...
    message = (
        f"Parent item deleted successfully " f"(including {child_count} child items)"
    )
