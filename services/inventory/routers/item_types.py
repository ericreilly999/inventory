...
from shared.models.item import ItemType, ParentItem, ChildItem
from shared.logging.config import get_logger
from ..schemas import (
    ItemTypeCreate, ItemTypeUpdate, ItemTypeResponse, MessageResponse
)
from ..dependencies import (
    get_current_user, require_inventory_read, require_inventory_write,
    require_inventory_admin, get_item_type_or_404
)
...
@router.post("/", response_model=ItemTypeResponse, dependencies=[Depends(require_inventory_admin)])
async def create_item_type(
    ...
    existing_type = db.query(ItemType).filter(ItemType.name == item_type_data.name).first()
    if existing_type:
        raise HTTPException(
...
@router.get("/", response_model=List[ItemTypeResponse], dependencies=[Depends(require_inventory_read)])
...
@router.get("/{type_id}", response_model=ItemTypeResponse, dependencies=[Depends(require_inventory_read)])
...
@router.put("/{type_id}", response_model=ItemTypeResponse, dependencies=[Depends(require_inventory_write)])
async def update_item_type(
    ...
    if item_type_data.name and item_type_data.name != item_type.name:
        existing = db.query(ItemType).filter(
            ItemType.name == item_type_data.name,
            ItemType.id != type_id
        ).first()
        if existing:
            raise HTTPException(
...
@router.delete("/{type_id}", response_model=MessageResponse, dependencies=[Depends(require_inventory_admin)])
async def delete_item_type(
    ...
    parent_items_count = db.query(ParentItem).filter(ParentItem.item_type_id == type_id).count()
    child_items_count = db.query(ChildItem).filter(ChildItem.item_type_id == type_id).count()
    
    if parent_items_count > 0 or child_items_count > 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Cannot delete item type: {parent_items_count + child_items_count} items are using this type"
        )
