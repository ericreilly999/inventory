...
class TestAPIErrorResponseConsistencyProperties:
        ...
        **Validates: Requirements 7.4**
        ...
        if error_type == 'authentication_error':
            ...
            with patch('httpx.AsyncClient.request') as mock_request:
                ...
                token_payload = {"sub": "test-user", "username": "test", "role": "user"}
    ...
    def test_rate_limit_error_consistency(self, user_data):
        ...
        with patch('services.api_gateway.middleware.rate_limit_middleware.RateLimitMiddleware._is_rate_limited') as mock_rate_limit:
    ...
    @given(
        ...
        endpoint=st.sampled_from(['/api/v1/items/parent', '/api/v1/locations', '/api/v1/users'])
    )
    @settings(max_examples=10)
    def test_invalid_token_error_consistency(self, invalid_token, endpoint):
        ...
        assert "invalid" in error_data["error"]["message"].lower() or "expired" in error_data["error"]["message"].lower()