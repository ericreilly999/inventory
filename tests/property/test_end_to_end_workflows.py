...
class InventorySystemStateMachine(RuleBasedStateMachine):
    ...
    @rule(target=location_types, name=valid_name(), description=valid_description())
    ...
    @rule(target=locations, location_type_id=location_types, name=valid_name(), description=valid_description())
    ...
    @rule(target=item_types, name=valid_name(), description=valid_description(), category=st.sampled_from(list(ItemCategory)))
    ...
    def create_parent_item(self, name, description, user_id, location_id):
        ...
        parent_item_types = [it for it in self.item_types.values() if it.category == ItemCategory.PARENT]
    ...
    def create_child_item(self, name, description, user_id):
        ...
        child_item_types = [it for it in self.item_types.values() if it.category == ItemCategory.CHILD]
    ...
    def parent_items_have_valid_locations(self):
        ...
        for item_id, parent_item in self.parent_items.items():
            ...
            assert self.item_locations[item_id] == parent_item.current_location_id, \
    ...
    def child_assignment_uniqueness(self):
        ...
        for child_id, parent_id in self.child_assignments.items():
            if parent_id is not None:
                assert child_id not in assigned_children, \
    ...
    def location_item_counts_accurate(self):
        ...
        for location_id in self.locations:
            ...
            assert self.location_item_counts[location_id] == expected_count, \
    ...
    def assignment_history_consistency(self):
        ...
        for child_id, assignments in child_assignments_history.items():
            if child_id in self.child_items:
                ...
                sorted_assignments = sorted(assignments, key=lambda a: self.assignment_history.index(a))
                if sorted_assignments:
                    last_assignment = sorted_assignments[-1]
                    current_parent = self.child_items[child_id].parent_item_id
                    assert last_assignment.to_parent_item_id == current_parent, \
    ...
    def referential_integrity(self):
        ...
        for user in self.users.values():
            assert user.role_id in self.roles, f"User {user.id} has invalid role {user.role_id}"
...
class TestEndToEndWorkflows:
        ...
        **Validates: Requirements 1.3, 1.4, 2.1**
        ...
        for item_id, item_data in items.items():
            ...
            if item_moves:
                ...
                assert item_data['current_location'] == last_move['to_location'], \
                ...
                for i in range(1, len(item_moves)):
                    ...
                    assert prev_move['to_location'] == curr_move['from_location'], \
    ...
    @settings(max_examples=20)
    def test_child_item_assignment_workflow(self, num_parents, num_children, num_assignments):
        """
        ...
        **Validates: Requirements 2.2, 2.3**
        ...
        for child_id, child_data in child_items.items():
            child_assignments = [a for a in assignment_history if a['child_id'] == child_id]
            ...
            if child_assignments:
                ...
                assert child_data['current_parent'] == last_assignment['to_parent'], \
    ...
    @given(
        operations=st.lists(
            st.one_of(
                ...
                st.tuples(st.just('move_item'), st.integers(min_value=0, max_value=9), valid_uuid_string()),
                st.tuples(st.just('assign_child'), st.integers(min_value=0, max_value=9), st.integers(min_value=0, max_value=9)),
        ...
        for operation in operations:
            ...
            try:
                if op_type == 'create_location':
                    ...
                    location_id = operation[2] if operation[2] in locations else locations[0]
                    item_id = str(uuid.uuid4())
                    ...
                    new_location = operation[2] if operation[2] in locations else locations[0]
