...
def test_assignment_history_tracking_property(test_db_session, data):
    ...
    **Validates: Requirements 9.4, 9.5**
    ...
    assert before_assignment_time <= assignment_history.assigned_at <= after_assignment_time
    ...
    history_records = test_db_session.query(AssignmentHistory).filter(
    ...
    from_parent_assignments = test_db_session.query(AssignmentHistory).filter(
        AssignmentHistory.from_parent_item_id == parent_item_1.id
    ).all()
    assert len(from_parent_assignments) == 1
    
    # 10. Assignment history can be queried by parent item (to)
    to_parent_assignments = test_db_session.query(AssignmentHistory).filter(
        AssignmentHistory.to_parent_item_id == parent_item_2.id
    ).all()
    assert len(to_parent_assignments) == 1
    
    # 11. Assignment history can be queried by date range
    date_filtered_assignments = test_db_session.query(AssignmentHistory).filter(
...
def test_initial_assignment_history_tracking(test_db_session, data):
    ...
    **Validates: Requirements 9.4**
    ...
    # 6. Contains timestamp within reasonable bounds
    assert before_assignment_time <= initial_assignment_history.assigned_at <= after_assignment_time
...
def test_multiple_assignment_history_chronological_order(test_db_session, data):
    ...
    **Validates: Requirements 9.5**
    ...
    for i, assignment_time in enumerate(assignment_times):
        ...
        assignment_history = AssignmentHistory(
            child_item_id=child_item.id,
            from_parent_item_id=current_from.id if i > 0 else None,  # First assignment has no "from"
    ...
    ordered_assignments = test_db_session.query(AssignmentHistory).filter(
        AssignmentHistory.child_item_id == child_item.id
    ...
    for i in range(len(ordered_assignments) - 1):
        assert ordered_assignments[i].assigned_at >= ordered_assignments[i + 1].assigned_at
...
def test_assignment_history_filtering_by_date_range(test_db_session, data):
    ...
    **Validates: Requirements 9.5**
    ...
    test_db_session.add_all([old_assignment, future_assignment, recent_assignment1, recent_assignment2])
    test_db_session.commit()
    ...
    filtered_assignments = test_db_session.query(AssignmentHistory).filter(
        AssignmentHistory.child_item_id == child_item.id,
    ...
    assert recent_assignment2.id in [assignment.id for assignment in filtered_assignments]
    assert recent_assignment1.id in [assignment.id for assignment in filtered_assignments]
    assert old_assignment.id not in [assignment.id for assignment in filtered_assignments]
    assert future_assignment.id not in [assignment.id for assignment in filtered_assignments]
