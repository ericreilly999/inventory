    ...
    **Validates: Requirements 1.3, 1.4**
    ...
    try:
        ...
        for parent_item in parent_items:
            ...
            session.refresh(parent_item)
            updated_item = session.query(ParentItem).filter(
                ParentItem.id == parent_item.id
            ).first()
        ...
        all_items_at_location2 = session.query(ParentItem).filter(
            ParentItem.current_location_id == location2.id
        ).all()
        ...
        items_at_location1 = session.query(ParentItem).filter(
            ParentItem.current_location_id == location1.id
        ).all()
        ...
        final_items_at_location1 = session.query(ParentItem).filter(
            ParentItem.current_location_id == location1.id
        ).all()
