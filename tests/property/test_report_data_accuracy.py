...
def test_report_data_accuracy_property(location_configs):
    ...
    **Validates: Requirements 3.1, 3.3**
    ...
    try:
        ...
        actual_parent_type_count = (
            session.query(ParentItem)
            .filter(ParentItem.item_type_id == parent_item_type.id)
            .count()
        )

        actual_child_type_count = (
            session.query(ChildItem)
            .filter(ChildItem.item_type_id == child_item_type.id)
            .count()
        )
        ...
        assert (
            actual_parent_type_count
            == expected_totals["by_item_type"]["parent"]["parent_count"]
        ), (
        ...
        assert (
            actual_child_type_count
            == expected_totals["by_item_type"]["child"]["child_count"]
        ), (
        ...
        assert total_actual_parents == expected_totals["total_parent_items"], (
            f"Total parent items mismatch: expected "
            f"{expected_totals['total_parent_items']}, got {total_actual_parents}"
        )

        assert total_actual_children == expected_totals["total_child_items"], (
            f"Total child items mismatch: expected "
            f"{expected_totals['total_child_items']}, got {total_actual_children}"
        )
