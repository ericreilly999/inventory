...
from shared.models import (
    ...
    ParentItem, ChildItem, ItemType, ItemCategory, MoveHistory
)
...
def test_report_date_filtering_property(movement_configs, filter_start_days, filter_end_days):
    """
    ...
    **Validates: Requirements 3.2, 5.2**
    ...
    try:
        ...
        if filter_start_days > filter_end_days:
            filter_start_days, filter_end_days = filter_end_days, filter_start_days
        ...
        assert len(filtered_movements) == len(expected_filtered_movements), \
        ...
        for movement in all_movements:
            if movement not in filtered_movements:
                ...
                assert not (filter_start_date <= movement.moved_at <= filter_end_date), \
        ...
        all_movements_ordered = session.query(MoveHistory).order_by(
