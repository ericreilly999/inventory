...
def create_test_data(session):
    ...
    session.commit()
    return user, location, location_type, parent_item_type, child_item_type, parent_item
...
@settings(deadline=None, max_examples=5)  # Disable deadline and reduce examples for faster testing
def test_constraint_enforcement_property(location_name, location_type_name, item_type_name):
    """
    ...
    **Validates: Requirements 4.4, 4.5, 8.4**
    ...
    try:
        ...
        # Create test data
        user, location, location_type, parent_item_type, child_item_type, parent_item = create_test_data(session)
        ...
        try:
            ...
            assert False, "Expected IntegrityError when deleting location with assigned items"
        ...
        deleted_location_type = session.query(LocationType).filter_by(
            name=location_type_name + "_unused"
        ).first()
        ...
        # Verify it was deleted
        deleted_item_type = session.query(ItemType).filter_by(
            name=item_type_name + "_unused"
        ).first()
        ...
        assert deleted_location is None, "Location should be deleted after removing dependencies"
        ...
        deleted_location_type = session.query(LocationType).filter_by(id=location_type.id).first()
        assert deleted_location_type is None, "Location type should be deleted after removing dependencies"
...
def test_location_deletion_with_items_constraint(location_name):
    ...
    **Validates: Requirement 4.4**
    ...
    try:
        ...
        assert existing_location is not None, "Location should still exist after failed deletion"
...
def test_item_type_deletion_with_items_constraint(item_type_name):
    ...
    **Validates: Requirement 8.4**
    ...
    try:
        ...
        existing_item_type = session.query(ItemType).filter_by(id=parent_item_type.id).first()
        assert existing_item_type is not None, "Item type should still exist after failed deletion"
...
if __name__ == "__main__":
    ...
    test_constraint_enforcement_property("TestLocation", "TestLocationType", "TestItemType")
