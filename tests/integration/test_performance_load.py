...
class TestAPIPerformance:
    ...
    def test_concurrent_read_operations(self, perf_runner: PerformanceTestRunner):
        ...
        metrics = perf_runner.run_concurrent_requests(read_inventory, 20, max_workers=10)
    ...
    def test_mixed_read_write_operations(self, perf_runner: PerformanceTestRunner):
        ...
        metrics = perf_runner.run_concurrent_requests(mixed_operations, 15, max_workers=8)
        
        # Should handle mixed load reasonably well
        success_rate = metrics.success_count / (metrics.success_count + metrics.error_count)
        assert success_rate >= 0.6, f"Mixed operations success rate {success_rate:.2%} < 60%"
        
        # Throughput should be reasonable
        assert metrics.throughput > 0.5, f"Throughput {metrics.throughput:.2f} req/s < 0.5 req/s"
    ...
    def test_database_query_performance(self, perf_runner: PerformanceTestRunner):
        ...
        report_metrics = perf_runner.run_concurrent_requests(query_reports, 10, max_workers=5)
        ...
        history_metrics = perf_runner.run_concurrent_requests(query_move_history, 10, max_workers=5)
...
class TestLoadTesting:
    ...
    def test_burst_load_handling(self, perf_runner: PerformanceTestRunner):
        ...
        if burst_metrics.success_count + burst_metrics.error_count > 0:
            success_rate = burst_metrics.success_count / (burst_metrics.success_count + burst_metrics.error_count)
            assert success_rate >= 0.5, f"Burst load success rate {success_rate:.2%} < 50%"
    ...
    def test_error_rate_under_load(self, perf_runner: PerformanceTestRunner):
        ...
        if len(error_rates) >= 2:
            ...
            assert max_error_rate < 0.5, f"Error rate {max_error_rate:.2%} > 50% under load"
    ...
    def test_memory_leak_detection(self, perf_runner: PerformanceTestRunner):
        ...
        if len(response_times) >= 3:
            first_third_avg = statistics.mean(response_times[:len(response_times)//3])
            last_third_avg = statistics.mean(response_times[-len(response_times)//3:])
            
            if first_third_avg > 0:
                time_increase_ratio = last_third_avg / first_third_avg
                assert time_increase_ratio < 2.5, \
...
class TestConcurrencyHandling:
    ...
    def test_concurrent_item_creation(self, perf_runner: PerformanceTestRunner):
        ...
        def create_item(item_suffix):
            return perf_runner.make_request("POST", "/api/v1/items/parent", json={
        ...
        successful_creates = [r for r in results if r['success'] and r.get('status_code') in [200, 201]]
        ...
        if len(results) > 0:
            ...
            assert success_rate >= 0.3, f"Concurrent creation success rate {success_rate:.2%} < 30%"
    ...
    def test_concurrent_item_updates(self, perf_runner: PerformanceTestRunner):
        ...
        def update_item(update_suffix):
            return perf_runner.make_request("PUT", "/api/v1/items/parent/test-item-id", json={
        ...
        if len(results) > 0:
            ...
            assert len(non_error_results) >= len(results) * 0.2, \
